...
export default function RedEnvelRevRecordModal({setActive}: {setActive: (status: boolean) => void}) {
        ...
        <div className="w-[500px] bg-[#160133] relative flex justify-center items-center bg-opacity-80 h-full px-4">
            ...
            <div className="w-full rounded-[20px] ">
                <div className="p-[3px] bg-gradient-to-r from-[#DC9409]  via-[#F9E0A9] to-[#DAB32B] rounded-lg">
                    <MyGuildModalSimpleHeader header="紅包審核紀錄" />
                    <div className="bg-[#150F20] bg-opacity-94  rounded-b-lg gap-1 flex flex-col items-center flex flex-row items-start">
                        <div className="w-full grid grid-cols-4 divide-x bg-[#6736F9] mt-5 py-3">
                            <p className="text-[15px] text-white font-medium text-center">申請時間</p>
                            <p className="text-[15px] text-white font-medium text-center">紅包數量</p>
                            <p className="text-[15px] text-white font-medium text-center">每包點數</p>
                            <p className="text-[15px] text-white font-medium text-center">審核結果</p>
                        </div>
                        <div className="w-full h-[300px] overflow-auto hide-scrollbar">
                            {
                                ...
                                <div key={index} className="w-full px-2 flex flex-col justify-between items-center mt-3 gap-2">
                                    <div className="w-full grid grid-cols-4">
                                        <p className="text-white text-[13px] font-medium text-center">{record.appPeriod}</p>
                                        <p className="text-white text-[13px] font-medium text-center">{record.redEnvNum}</p>
                                        <p className="text-white text-[13px] font-medium text-center">{record.pointPerPack}</p>
                                        {
                                            record.auditResult ? 
                                            <p className="text-[#2AD41B] text-[13px] font-medium text-center">已成功</p> : <p className="text-white text-[13px] font-medium text-center"></p>
                                        }
                                    </div>
